...
public class RandomDecorator implements Decorator {
	...
	public void decorate(Resource resource, DecoratorContext context) {
		...
		if (ResourceUtils.isFile(resource)) {
			...
			// we sum random values on resources different than file. Another
			// option would be to declare a Formula on metric declaration
			context.saveMeasure(
					ExampleMetrics.RANDOM,
					MeasureUtils.sum(true,
							context.getChildrenMeasures(ExampleMetrics.RANDOM)));
		}
	}
