...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pd="http://xmlns.tibco.com/bw/process/2003">
	...
	<xsl:template match="/">
		<results>
			<xsl:call-template name="start" /> <!-- handling the Start activity (when there is no other starter)-->
	...
	<xsl:template name="start">
		...
		<xsl:if test="count(/pd:ProcessDefinition/pd:transition[pd:from = $activityName]) = 0 and count(//pd:starter) = 0">
			<result>
